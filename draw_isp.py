...
# 加载 .npz 文件
# data = np.load('./data_arrays_res50.npz')
data = np.load('ISP_RES50_EVAL.npz')
# print(data['eval_list'])
print(data['maked_loss'])
# # 通过键名访问数组
...
# # x 表示裁剪的百分比
# x = list(10*i for i in range(len(maked_acc[0])))
...
# # 设置标题
# ax1.set_title('Performance Comparison (ResNet50)', fontsize=16)
...
# # 保存为图像文件，适合论文使用
# plt.savefig('scatter_plot_res.png', dpi=300)  # 设置 dpi 为 300 确保高质量输出
# plt.close()