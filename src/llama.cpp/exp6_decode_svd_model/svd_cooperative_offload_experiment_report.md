...
### 9.4.1 2026-04-07 FFN 阶段补充对比

下面这组数据不是整池 `60-79` 的吞吐基线，而是为了回答“FFN 这一阶段本身到底多花在哪里”，把电脑端本地 FFN 和协同 FFN 放到同一粒度上做对比。

测试口径：

- 本地：电脑端绑核 `68-75`，`OMP_NUM_THREADS=8`
- 协同：客户端绑核 `68-75`，服务端绑核 `60-67`，两端都为 `OMP_NUM_THREADS=8`
- 本地看 `ffn_svd_total`
- 协同看 `rpc_send_total`、`rpc_wait_total`、`remote_compute_total`
...
- `rpc_wait_total` 已经包含 `remote_compute_total`
- 因此协同 FFN 的客户端墙钟时间应按 `32.916 + 737.821 = 770.737 ms` 计算
- 相比本地 FFN 的 `623.092 ms`，当前协同 FFN 多出 `147.645 ms`
- 这 `147.645 ms` 中：
  - 远端执行本身比本地多 `87.397 ms`
  - 发送与额外等待开销为 `60.248 ms`

这说明当前瓶颈已经更接近“FFN 被拆成 RPC 后的等待成本”，而不是“远端完全算不动 FFN”。
...
- 本轮中 `80%` 卸载最好
- `100%` 卸载反而慢于 `70% / 80%`
- 该表对应的当时代码仍然只在 `offload_rate > 0.5` 时触发真实协同，因此表中的 `50%` 实际上不会真正走远端尾部计算
- 截至 2026-04-07，这个阈值已修成 `offload_rate > 0.0`，因此当前代码里的 `50%` 已经会真实触发 partial-rank 协同
...
- 该表中的 `50%` 仍不是严格意义上的半卸载，而是接近本地参考值，因为实验当时代码条件是 `offload_rate > 0.5`
- 截至 2026-04-07，当前代码已经放宽为 `offload_rate > 0.0`
