...
static bool parse_svd_eval_params(
        int argc,
        char ** argv,
        svd_eval_params & svd_params,
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--svd-offload-endpoint") {
            if (i + 1 >= argc) {
                LOG_ERR("%s: missing value for %s\n", __func__, arg.c_str());
                return false;
            }
            svd_params.offload_endpoint = argv[++i];
            continue;
        }
        if (arg == "--svd-offload-rates") {
            if (i + 1 >= argc) {
                LOG_ERR("%s: missing value for %s\n", __func__, arg.c_str());
                return false;
            }
            svd_params.offload_rates_arg = argv[++i];
            continue;
        }
        if (arg == "--svd-offload-timeout") {
            if (i + 1 >= argc) {
                LOG_ERR("%s: missing value for %s\n", __func__, arg.c_str());
                return false;
            }
...
static common_init_result common_init_from_params_svd(
        common_params & params,
        const svd_eval_params & svd_params) {
    ...
    if (!offload_rates.empty()) {
        cparams.svd_offload_rates = offload_rates.data();
        cparams.svd_offload_rate_count = static_cast<uint32_t>(offload_rates.size());
    }

    std::string offload_host;
    uint16_t offload_port = 0;
    const bool has_endpoint = parse_host_port(svd_params.offload_endpoint, offload_host, offload_port);
    const bool has_rates = !offload_rates.empty();

    if (has_endpoint && has_rates) {
        cparams.svd_offload_enabled = true;
        cparams.svd_offload_host = offload_host.c_str();
        cparams.svd_offload_port = offload_port;
        cparams.svd_offload_timeout_ms = svd_params.offload_timeout_ms;
        LOG_INF("%s: SVD offload enabled: %s timeout_ms=%d rates=%zu\n",
                __func__, svd_params.offload_endpoint.c_str(),
                svd_params.offload_timeout_ms, offload_rates.size());
    } else if (has_rates) {
    ...
    result.model.reset(model);
    result.context.reset(ctx);
    return result;
}
...
static results_perplexity perplexity(llama_context * ctx, const common_params & params, const int32_t n_ctx) {
    if (params.ppl_stride > 0) {
        LOG_WRN("%s: ppl_stride is ignored in streaming SVD perplexity mode\n", __func__);
    }
    ...
    if (llama_vocab_get_add_eos(vocab)) {
        LOG_WRN("%s: add_eos vocab flag is ignored in streaming SVD perplexity mode\n", __func__);
    }
    if (!params.logits_file.empty()) {
        LOG_WRN("%s: logits_file output is not supported in streaming SVD perplexity mode\n", __func__);
    }
    ...
    if (tokens.size() < 2) {
        LOG_ERR("%s: you need at least 2 tokens to evaluate perplexity\n", __func__);
        return {std::move(tokens), 0., {}, {}};
    }

    std::vector<float> logit_history(tokens.size(), 0.0f);
    std::vector<float> prob_history(tokens.size(), 0.0f);

    const int n_vocab = llama_vocab_n_tokens(vocab);
    const int n_chunk_max = (int) ((tokens.size() - 1 + n_ctx - 1) / n_ctx);
    const int n_chunk = params.n_chunks < 0 ? n_chunk_max : std::min(params.n_chunks, n_chunk_max);

    if (n_chunk <= 0) {
        LOG_ERR("%s: no chunks selected for evaluation\n", __func__);
        return {std::move(tokens), 0., {}, {}};
    }

    LOG_INF("%s: calculating streaming perplexity over %d chunks, n_ctx=%d\n", __func__, n_chunk, n_ctx);
    ...
    for (int i = 0; i < n_chunk; ++i) {
        ...
        const int end = std::min<int>(start + n_ctx, (int) tokens.size());
        if (end - start < 2) {
            break;
        }
        ...
        for (int j = start; j < end - 1; ++j) {
            batch.n_tokens = 1;
            batch.token[0] = add_bos && j == start ? llama_vocab_bos(vocab) : tokens[j];
            batch.pos[0] = j - start;
            batch.n_seq_id[0] = 1;
            batch.seq_id[0][0] = 0;
            batch.logits[0] = 1;
            ...
            if (llama_decode(ctx, batch) != 0) {
                LOG_ERR("%s: failed to decode token at chunk=%d offset=%d\n", __func__, i, j - start);
                llama_batch_free(batch);
                return {std::move(tokens), -1, logit_history, prob_history};
            }

            const float * logits = llama_get_logits_ith(ctx, 0);
            if (logits == nullptr) {
                LOG_ERR("%s: llama_get_logits_ith returned nullptr at chunk=%d offset=%d\n", __func__, i, j - start);
                llama_batch_free(batch);
                return {std::move(tokens), -1, logit_history, prob_history};
            }

            const results_log_softmax results = log_softmax(n_vocab, logits, tokens[j + 1]);
            const double v = -results.log_softmax;
            nll += v;
            nll2 += v * v;
            logit_history[j] = results.logit;
            prob_history[j] = results.prob;
            ++count;
        }
...
int main(int argc, char ** argv) {
    ...
    if (!parse_svd_eval_params(argc, argv, svd_params, filtered_argv)) {
        return 1;
    }

    if (!common_params_parse((int) filtered_argv.size(), filtered_argv.data(), params, LLAMA_EXAMPLE_PERPLEXITY)) {
        return 1;
    }
    ...
    common_init_result llama_init = common_init_from_params_svd(params, svd_params);
